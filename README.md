...
The weak-data filtering stage now uses **trained DPR semantic retrieval** instead of BM25 lexical matching.

Yes — the iterative pipeline works as:
1. **Round 1 filtering**: run filter with an initial DPR checkpoint.
2. **Round 1 retraining**: train DPR on the round-1 filtered weak pairs.
3. **Round 2 filtering**: run filter again on the same raw weak pairs, but now using the **new DPR checkpoint trained in round 1**.
4. Repeat until retrieval metrics stop improving.

`zhiyuan/filter/dpr_dense_filter.py` keeps a weak pair only if the target weak document is retrieved by DPR in top-k for its weak query.

### Round 1 (initial DPR as judge)
```
python zhiyuan/filter/dpr_dense_filter.py \
  --dataset_name fiqa \
  --train_num 50 \
  --weak_num 100k \
  --exp_name llama_7b_100k_fixed_v4_best_llama_prompt_3 \
  --output_exp_name llama_7b_100k_fixed_v4_best_llama_prompt_3_dpr_r1_top30 \
  --model_path zhiyuan/retriever/dpr/train/output/no_aug/50/bert-large-uncased-v1-fiqa \
  --topk 30
```

Train DPR on round-1 filtered data:
```
python -m zhiyuan.retriever.dpr.train.train_sbert \
  --dataset_name fiqa \
  --train_num 50 \
  --weak_num 100k \
  --exp_name llama_7b_100k_fixed_v4_best_llama_prompt_3_dpr_r1_top30
```

### Round 2 (newly trained DPR as judge)
```
python zhiyuan/filter/dpr_dense_filter.py \
  --dataset_name fiqa \
  --train_num 50 \
  --weak_num 100k \
  --exp_name llama_7b_100k_fixed_v4_best_llama_prompt_3 \
  --output_exp_name llama_7b_100k_fixed_v4_best_llama_prompt_3_dpr_r2_top30 \
  --model_path zhiyuan/retriever/dpr/train/output/llama_7b_100k_fixed_v4_best_llama_prompt_3_dpr_r1_top30/50/bert-large-uncased-v1-fiqa \
  --topk 30
```

The filter also exports `*_scores.tsv` with per-pair soft score `1/log2(rank+1)`, which can be used to set softer quality thresholds in later rounds.

### One-command iterative loop (filter + train each round)
If you want the full loop to happen automatically, run:
```
python zhiyuan/filter/run_iterative_dpr_filter.py \
  ...
  --rounds 2
```
This script runs DPR filtering then DPR training in each round, and automatically uses the newly trained DPR checkpoint from round *r* as the filtering model for round *r+1*.
## Official Code
